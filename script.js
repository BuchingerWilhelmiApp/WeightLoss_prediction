...
function displayResults(age, height, weightBaseline, timeline, gender, resultParagraph) {
    ...
    let yNew = -1.7433 - 0.0124 * weightBaseline + 0.7392 * mb + 1.1292 * timelineTrans - 0.5049 * mb * timelineTrans;

    // Arrondir à 2 décimales
    yNew = yNew.toFixed(2);

    // Affichage des résultats
    resultParagraph.innerHTML = `Your basal metabolism is ${mbCaloriesConverted.toFixed(2)} calories.<br>Your Weight Loss is : ${yNew} kg.`;
...
function plotCurve(age, height, weight_baseline, timeline) {
    ...
    for (var i = 0; i <= timeline; i++) {
        ...
        y_new_m.push(i === 0 ? 0 : (-1.7433 - 0.0124 * weight_baseline + 0.7392 * MB_men + 1.1292 * Timeline_trans[i] - 0.5049 * MB_men * Timeline_trans[i]));
        y_new_f.push(i === 0 ? 0 : (-1.7433 - 0.0124 * weight_baseline + 0.7392 * MB_women + 1.1292 * Timeline_trans[i] - 0.5049 * MB_women * Timeline_trans[i]));
    }
